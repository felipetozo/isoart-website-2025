                    ...
                    <div className={styles['category-products-grid']}>
                        {categoryData.products.map((product) => (
                            <article key={product.id} className={styles['category-products-grid-card']}>
                                <Link href={`/${lang}/categorias/${categoryData.slug}/${product.slug}`}>
...
export async function generateStaticParams() {
    const allCategories = (menuData as CategoryData[]).map((category: CategoryData) => ({
        lang: 'pt-BR',
        category: category.slug,
    }));

    // Adicionar versões em inglês e espanhol
    const enCategories = (menuData as CategoryData[]).map((category: CategoryData) => ({
        lang: 'en',
        category: category.slug,
    }));

    const esCategories = (menuData as CategoryData[]).map((category: CategoryData) => ({
        lang: 'es',
        category: category.slug,
    }));

    return [...allCategories, ...enCategories, ...esCategories];
}
